...
function ReconNg() {
  ...
  const handleScan = async () => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/reconng`, { target });
  ...
  return (
    <div ref={containerRef} className="p-4 sm:p-6 md:p-8 max-w-7xl mx-auto space-y-6">
      <div className="gsap-header flex items-center gap-4 opacity-0">
        ...
        <div>
          <h2 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400">Recon-ng</h2>
          <p className="text-cyber-primary font-mono text-xs tracking-[0.2em] uppercase mt-1">Modular Reconnaissance Framework</p>
        </div>
      </div>
      ...
      <div className="gsap-guide glass-panel p-4 border border-white/5 bg-white/5 rounded-lg opacity-0">
        ...
        <p className="text-gray-400 text-sm leading-relaxed">
          Enter a target (domain or keyword) to perform modular reconnaissance.
          This tool aggregates data from various sources to build a target profile.
        </p>
      </div>

      {/* Input Panel */}
      <div className="gsap-input glass-panel p-8 border border-white/10 relative overflow-hidden opacity-0">
        <label className="block text-xs font-mono text-gray-400 uppercase tracking-wider mb-3">Target (Domain/Keyword)</label>
