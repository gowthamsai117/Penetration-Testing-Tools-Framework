...
function TheHarvester() {
  ...
  const handleScan = async () => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/theharvester`, { domain });
  ...
  return (
    <div ref={containerRef} className="p-4 sm:p-6 md:p-8 max-w-7xl mx-auto space-y-6">
      <div className="gsap-header flex items-center gap-4 opacity-0">
        ...
        <div>
          <h2 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400">theHarvester</h2>
          <p className="text-cyber-primary font-mono text-xs tracking-[0.2em] uppercase mt-1">OSINT Email & Subdomain Gathering</p>
        </div>
      </div>
      ...
      <div className="gsap-guide glass-panel p-4 border border-white/5 bg-white/5 rounded-lg opacity-0">
        ...
        <p className="text-gray-400 text-sm leading-relaxed">
          Enter a domain name (e.g., <code className="text-cyber-primary bg-black/30 px-1 rounded">example.com</code>) to gather emails, subdomains, hosts, and open ports from various public sources.
        </p>
      </div>
